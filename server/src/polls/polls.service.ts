...
export class PollsService {
	...
	async createPoll(fields: CreatePollFields) {
		...
		this.logger.debug(`Creating token string pollID: ${createdPoll.id} and userID ${userID}`);
	...
	async submitRankings(rankingsData: SubmitRankingsFields): Promise<Poll> {
		...
		if (!hasPollStarted) {
			throw new BadRequestException('Participants cannot rank until poll has started.');
		}
