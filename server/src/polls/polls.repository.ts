...
export class PollsRepository {
	...
	async addNomination({ pollID, nominationID, nomination }: AddNominationData): Promise<Poll> {
		this.logger.log(`Attempting to add a nominationID/nomination: ${nominationID}/${nomination.text} to pollID: ${pollID}`);
		const key = `polls:${pollID}`;
	...
	async removeNomination(pollID: string, nominationID: string): Promise<Poll> {
		...
		try {
			...
			throw new InternalServerErrorException(`Failed to remove nominationID: ${nominationID} from pollID: ${pollID}`);
		}
	}
	...
	async startPoll(pollID: string): Promise<Poll> {
		...
		try {
			...
			this.logger.debug(`Failed set hasStarted for poll: ${pollID}`, e);
	...
	async addParticipantRankings({ pollID, userID, rankings }: AddParticipantRankingsData): Promise<Poll> {
		...
		try {
			...
			this.logger.error(`Failed to add rankings for userID: ${userID} to pollID: ${pollID}`, rankings);
			throw new InternalServerErrorException('There was an error starting the poll');
		}
	}
	...
	async addResults(pollID: string, results: Results): Promise<Poll> {
		...
		try {
			...
			this.logger.error(`Failed to add results to pollID: ${pollID}`, results, e);
	...
	async deletePoll(pollID: string): Promise<void> {
		const key = `polls:${pollID}`;
		this.logger.log(`Deleting pollID: ${pollID}`);
		try {
			await this.redisClient.send_command('JSON.DEL', key);
		} catch (e) {
			this.logger.error(`Failed to delete pollID: ${pollID}`, e);
			throw new InternalServerErrorException(`Failed to delete pollID: ${pollID}`);
		}
	}
} 
